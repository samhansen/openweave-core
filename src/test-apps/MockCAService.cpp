...
{
    ReqType = WeaveCertProvEngine::kReqType_NotSpecified;
...
WEAVE_ERROR GetCertificateRequestMessage::Decode(PacketBuffer *msgBuf)
{
    ...
    // Request Type.
    {
        ...
        VerifyOrExit(ReqType == WeaveCertProvEngine::kReqType_GetInitialOpDeviceCert ||
                     ReqType == WeaveCertProvEngine::kReqType_RotateOpDeviceCert, err = WEAVE_ERROR_INVALID_ARGUMENT);

        err = reader.Next();
        SuccessOrExit(err);
    }

    // Request Authorization Information (optional).
    if (reader.GetType() == kTLVType_Structure && reader.GetTag() == ProfileTag(kWeaveProfile_Security, kTag_GetCertAuthorizeInfo))
    {
        ...
        // Verify that at least one of these elements is present.
        VerifyOrExit((reader.GetType() == kTLVType_ByteString && reader.GetTag() == ContextTag(kTag_GetCertAuthorizeInfo_PairingToken)) ||
                     (reader.GetType() == kTLVType_ByteString && reader.GetTag() == ContextTag(kTag_GetCertAuthorizeInfo_PairingInitData)),
        ...
        {
            // Verify that the second element can only be PairingInitData.
            VerifyOrExit((reader.GetType() == kTLVType_ByteString && reader.GetTag() == ContextTag(kTag_GetCertAuthorizeInfo_PairingInitData)),
                         err = WEAVE_ERROR_UNEXPECTED_TLV_ELEMENT);
        }
    ...
    if (reader.GetType() == kTLVType_Array && reader.GetTag() == ContextTag(kTag_GetCertReqMsg_OpRelatedCerts))
    {
        ...
        VerifyOrExit(ReqType != WeaveCertProvEngine::kReqType_GetInitialOpDeviceCert, err = WEAVE_ERROR_UNEXPECTED_TLV_ELEMENT);

        err = mOperationalCertSet.LoadCerts(reader, kDecodeFlag_GenerateTBSHash);
        SuccessOrExit(err);
    ...
    if (reader.GetType() == kTLVType_Structure && reader.GetTag() == ProfileTag(kWeaveProfile_Security, kTag_ManufAttestInfo_Weave))
    {
        err = reader.EnterContainer(outerContainer2);
        SuccessOrExit(err);
    ...
    else if (reader.GetType() == kTLVType_Structure && reader.GetTag() == ProfileTag(kWeaveProfile_Security, kTag_ManufAttestInfo_X509))
    {
        // TODO: Implement
    ...
    if (ManufAttestInfoPresent())
    {
        ...
        else if (reader.GetType() == kTLVType_Structure && reader.GetTag() == ContextTag(kTag_GetCertReqMsg_ManufAttestSig_RSA))
        {
            // TODO: Implement
            ExitNow(err = WEAVE_ERROR_NOT_IMPLEMENTED);
        }
...
WEAVE_ERROR MockCAService::SendStatusReport(nl::Weave::ExchangeContext *ec)
{
    ...
    statusReport.mStatusCode = Security::kStatusCode_UnathorizedGetCertRequest;
...
WEAVE_ERROR MockCAService::ProcessGetCertificateRequest(PacketBuffer *msgBuf, GetCertificateRequestMessage & msg)
{
    ...
    {
        VerifyOrExit(msg.ManufAttestInfoPresent(), err = WEAVE_ERROR_INVALID_ARGUMENT);
    }

    // Validate Manufacturer Attestation Information if present.
    if (msg.ManufAttestInfoPresent())
    {
