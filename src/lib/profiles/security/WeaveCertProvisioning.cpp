...
WEAVE_ERROR WeaveCertProvEngine::GenerateGetCertificateRequest(PacketBuffer * msgBuf, uint8_t reqType, bool doManufAttest)
{
    ...
    err = mOpAuthDelegate->EncodeCert(writer, ContextTag(kTag_GetCertReqMsg_OpDeviceCert));
    SuccessOrExit(err);

    // Call the delegate to write the local node Weave operational related certificate.
    err = mOpAuthDelegate->EncodeRelatedCerts(writer, ContextTag(kTag_GetCertReqMsg_OpRelatedCerts));
    SuccessOrExit(err);

    // Call the delegate to write the local node manufacturer attestation information.
    if (doManufAttest)
    {
        err = mManufAttestDelegate->EncodeInfo(writer);
        SuccessOrExit(err);
    }
    ...
    err = mOpAuthDelegate->EncodeSig(tbsHash, SHA256::kHashLength, writer, ContextTag(kTag_GetCertReqMsg_OpDeviceSig_ECDSA));
    SuccessOrExit(err);

    // Generate and encode manufacturer attestation device signature.
    if (doManufAttest)
    {
        err = mManufAttestDelegate->EncodeSig(tbsHash, SHA256::kHashLength, writer);
        SuccessOrExit(err);
    }
