...
public:
     ...
     */
    virtual WEAVE_ERROR EncodeCert(TLVWriter & writer, uint64_t tag) = 0;
     ...
     */
    virtual WEAVE_ERROR EncodeRelatedCerts(TLVWriter & writer, uint64_t tag) = 0;
     ...
     */
    virtual WEAVE_ERROR EncodeSig(const uint8_t * hash, uint8_t hashLen, TLVWriter & writer, uint64_t tag) = 0;
};
...
public:
     ...
     */
    virtual WEAVE_ERROR EncodeInfo(TLVWriter & writer) = 0;
     ...
     * RSASignature, or custom structure to the supplied TLV writer using one of the
     * following tags:
     ...
     * can be useful for implementing this method.
     */
    virtual WEAVE_ERROR EncodeSig(const uint8_t * hash, uint8_t hashLen, TLVWriter & writer) = 0;
};
